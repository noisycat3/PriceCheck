...
void MenuManager::registerHooks(std::shared_ptr<GameWrapper> gameWrapper)
{
	gameWrapper->HookEventWithCallerPost<MenuStackWrapper>(HOOK_MENU_CHANGED,
		[this](MenuStackWrapper caller, void* params, std::string eventName) { handleMenuChanged(caller); });
}

void MenuManager::unregisterHooks(std::shared_ptr<GameWrapper> gameWrapper)
{
...
void MenuManager::handleMenuChanged(MenuStackWrapper menuStack)
{
    ...
    // Add missing
    while (stack.size() < srcStack.size())
        pushMenu(convertedTemp[stack.size()]);
}
